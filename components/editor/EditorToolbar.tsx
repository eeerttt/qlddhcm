...
import { MousePointer2, Plus, Move, Maximize, Search, Map as MapIcon, Grid, Keyboard, Magnet, Trash2, Undo2, Redo2 } from 'lucide-react';
...
const EditorToolbar: React.FC<EditorToolbarProps> = ({
    ...
    return (
        <div className="w-16 bg-slate-900 border-r border-slate-800 flex flex-col items-center py-6 gap-3 z-20 shadow-2xl">
            ...
            <div className="w-8 h-px bg-slate-800 my-2" />
            <button onClick={() => setShowBasemap(!showBasemap)} className={`p-3 rounded-xl transition-all ${showBasemap ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-800'}`} title="Bật/Tắt bản đồ nền vệ tinh"><MapIcon size={20}/></button>
